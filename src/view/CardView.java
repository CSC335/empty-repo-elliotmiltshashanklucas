...
import model.Theme;
/**
 * CardView is an ImageView representing a card in a game, capable of displaying different
 * images based on the card's state and animating flips between these states.
 * It observes changes in the card and theme models to update its appearance.
 */
public class CardView extends ImageView implements Observer{
	private Card card;
	private Theme theme = Theme.getTheme();
    /**
     * Constructs a new CardView with a specified card.
     * Sets up the image view properties, registers as an observer to the card and theme,
     * and initializes the card image based on its current state.
	...
	/**
     * Updates the image displayed by the ImageView based on whether the card is flipped.
     * @param isFlipped true if the card is flipped (showing its face), false if showing its back
     */
    ...
    /**
     * Called when an observed object changes. This method handles flipping animations
     * and updates the card's image according to the new state.
     */
	public void update() {
